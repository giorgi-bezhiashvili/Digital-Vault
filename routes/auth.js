...
const generateAccessToken = (user) => {
    return jwt.sign({ userName: user.userName , id: user._id}, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "15m" });
};
...
router.post("/login", validate(schemas.login), async (req, res) => {
    ...
    if (user.password === "google-auth-user") return res.status(400).json({ message: "Use Google Login" });
